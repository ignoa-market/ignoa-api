...
// 10초 동안 목표 요청률까지 올린 뒤, 같은 상품에 5분 동안 일정하게 보낸다.
export const options = {
    scenarios: {
        bid_contention: {
            ...
            startRate: 0,
            timeUnit: '1s',
            preAllocatedVUs: Math.ceil(TARGET_RATE * 1.5),
            maxVUs: 500,
            stages: [
                {duration: '10s', target: TARGET_RATE},
                {duration: '300s', target: TARGET_RATE},
            ],
            gracefulStop: '10s',
        },
    },
    thresholds: {
        'http_req_failed{scenario:bid_contention}': ['rate==0'],
        'dropped_iterations{scenario:bid_contention}': ['count==0'],
    },
};
...
function validateEnvironment() {
    ...
    if (!__ENV.TARGET_RATE) missing.push('TARGET_RATE');
    ...
    if (!Number.isInteger(TARGET_RATE) || TARGET_RATE <= 0) {
        throw new Error('TARGET_RATE는 0보다 큰 정수여야 합니다');
    }
}
